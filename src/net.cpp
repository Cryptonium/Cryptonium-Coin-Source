...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Cryptonium " + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
        /* miniupnpc 1.5 */
...
static const char *strDNSSeed[][2] = {
     {"cryptonium node", "test.cryptonium.org"},
     {"cryptonium node", "test1.cryptonium.org"},
     {"cryptonium node", "test2.cryptonium.org"},
	 {"cryptonium dev test", "cryptonium.no-ip.biz"},
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Cryptonium is probably already running."), addrBind.ToString().c_str());
        else
